 ...
 */
public class Sender {
    static final byte windowSize = 4, packetsToDrop = 2;
    static byte sequenceNumberSize = 10, currentSequence = 0;
    static DatagramPacket sendPkt, rcvPkt;
    static long[] timers = new long[windowSize];
    InetAddress IPAddress;
    static Timer timer = new Timer();
    static byte resendIndex = -1;
    static String[] window = new String[windowSize];;
    int port;
    ...
    public static void main(String[] args) throws SocketException, UnknownHostException, IOException {
        ...
        timer.schedule(new TimerTask() {
            @Override
            public void run() {
                if(allPacketsDelivered(window))
                    return;
                resendIndex = needToResendPacket();
                if(resendIndex != -1) {
                    System.out.println(resendIndex);
                    try {
    ...
    public static boolean canSendPacket(String[] window) {
        for(int i = 0; i < window.length; i++) {
            if(window[i] == null || (!window[i].equals("-") && window[i].indexOf("*") == -1 && window[i].indexOf("#") == -1))
                return true;
        }
     ...
     * @param status            Status to update the sequence number with.  
     *                          True indicates ACKed, false indicates sent, but
     *                          not ACKed.
